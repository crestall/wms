    ...
    public function createOrderItemsArray(array $items, $order_id = 0, $store_order = false)
    {
        ...
        foreach($items as $oid => $order_items)
        {
            ...
            foreach($order_items as $id => $details)
            {
                ...
                {
                    $item_error = true;
                    $item_error_string .= "<li>Only whole positive numbers can be selected for pick quantities</li>";
                }
                else
                {
                    $f_locations = array();
                    ...
                    else
                    {
                        ...
                        if( $item['whole_bays'] > 0 )
                        {
                            ...
                            //echo "Locations<pre>",print_r($locations),"</pre>";die();
                ...
                {
                    $item_error = true;
                    $item_error_string .= "<li>Could not find a location for $item_name for a quantity of $pick_count</li>";
                }
                $values[] = array(
                    'item_id'               => $id,
                    'qty'                   => $details['qty'],
