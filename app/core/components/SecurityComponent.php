...
class SecurityComponent extends Component{
    ...
    public function startup(){
        ...
        if(($this->request->isPost()) && $this->config["validateForm"]){
            if(!$this->form($this->config["form"])){
                Logger::log("Form Validation", "Vaslidate form failure", __FILE__, __LINE__);
                return $this->invalidRequest();
            }
        }
    ...
    */
            ...
            if (!empty($this->config[$key])) {
                if (in_array($this->request->param('action'), $this->config[$key], true) || $this->config[$key] === ['*']) {
                    if (!$this->request->{"is" . $method}()) {
                        Logger::log("Request Required", $key." has failed the ".$method, __FILE__, __LINE__);
                        return false;
                    }
                }
            }
     ...
     * @return mixed
     * @throws Exception - not any more: returns 400 page
     */
    ...
    public function form($config){
        ...
        /*
        Removed. Need to find another way of doing this, or is the csrfToken enough?
