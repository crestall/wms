                                    ...
                                    <form action="#" id="form-update-user-info" method="post" >
										<div class="form-group">
											<div class="block-section text-center">
												<img src="<?php echo $info["image"];?>" class="img-circle profile-pic-lg">
												<h3><strong><?php echo $info["name"];?></strong></h3>
												<h4>
													<span class="label label-default"><?php echo ucfirst($info["role"]);?></span>
												</h4>
											</div>
										</div>
                                        <div class="form-group">
                                            <label>User Name</label>
                                            <input dir="auto" type="text" name="name" value="<?php echo $info["name"];?>" class="form-control" required maxlength="30" placeholder="Name..">
											<p class="help-block"><em>The maximum number of characters allowed is <strong>30</strong></em></p>
                                        </div>
										<div class="form-group">
                                            <label>Email</label>
                                             <input type="email" value="<?php echo $this->encodeHTML($info["email"]); ?>" disabled class="form-control" maxlength="50" placeholder="Email..">
                                            ...
                                            <select name="role" class="form-control" size="1">
                                                ...
                                                <?php foreach(['admin', 'user'] as $role){ ?>
                                                    <?php if($role === $info['role']) { ?>
                                                        <option selected value="<?php echo $info['role']; ?>"><?php echo ucfirst($info['role']); ?></option>
                                                    <?php } else { ?>
                                                        <option  value="<?php echo $info['role']; ?>"><?php echo ucfirst($info['role']); ?></option>
                                                    <?php } ?>
                                                <?php } ?>
                                            </select>
                                        </div>
